...
values ('Входящие', 1), ('Учеба', 2), ('Работа', 3), ('Домашние дела', 2), ('Авто', 1), ('Спорт', 3), ('Животные', 3), ('Путешествия', 1);
...
insert into tasks set status_task = 0, title = 'Выполнить тестовое задание', deadline = '2019-06-09', user_id = 3, project_id = 3;
...
insert into tasks set status_task = 0, title = 'Выгулять собаку', deadline = '2019-06-13', user_id = 3, project_id = 7;
insert into tasks set status_task = 0, title = 'Помыть посуду', deadline = '2019-06-12', user_id = 2, project_id = 4;
insert into tasks set status_task = 1, title = 'Помыть машину', deadline = '2019-02-22', user_id = 1, project_id = 5;
insert into tasks set status_task = 0, title = 'Заказать билеты', deadline = '2019-06-14', user_id = 1, project_id = 8;
insert into tasks set status_task = 0, title = 'Купить абонемент в бассеин', deadline = '2019-06-25', user_id = 3, project_id = 6;

/*получить список из всех проектов для одного пользователя. */
select p.project_id, p.project, p.user_id as user_projects from projects p where p.user_id = 2;
...
select p.project_id, p.project, count(t.id) as count_tasks from projects p left join tasks t on p.project_id = t.project_id where p.user_id = 2 group by p.project_id;
